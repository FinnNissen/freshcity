	...
	class Shop_Orders extends Backend_Controller
	{
		...
		public function __construct()
		{
			...
			if (post('find_product_mode'))
			{
				...
				$this->list_search_fields = array('@name', '@sku', '(select group_concat(sku) from shop_products sku_list where sku_list.product_id is not null and sku_list.product_id=@id)', '(select group_concat(sku) from shop_option_matrix_records where product_id=@id)');
		...
		{
			return Shop_Product::create()->where('grouped is null')->where('disable_completely is null or disable_completely=0');
		}
